...
public class LabeledGraph<E, T> {
	...
	public ArrayList<String> DijkstraAlgorithm(E start, E end) {
		PQ<Vertex> toVisit = new PQ<Vertex>();
		...
		while(toVisit.size() != 0) {
			if(!curr.info.equals(end)) {
				for(Edge e :curr.edges) {
					if(visited.contains(e)){
						toVisit.put(e.getNeighbor(curr), (int)distances.get(curr) + (int)e.label);
						if(distances.get(e.getNeighbor(curr)) > (int)distances.get(curr) + (int)e.label) {
							distances.remove(e.getNeighbor(curr));
						...
						toVisit.put(e.getNeighbor(curr), (int)distances.get(curr) + (int)e.label);// this line is weird and i dont know how to fix it
						distances.put(e.getNeighbor(curr), (int)distances.get(curr) + (int)e.label);
					}
				}
				curr = toVisit.pop();
				...
				for(Vertex v: backTraceWithEdges(leadsTo, getVertex(start), getVertex(end))) {
					out.add((String)v.info);
				}
			}
		}
		return null;
	...
	public ArrayList<Vertex> backTraceWithEdges(HashMap<Vertex, Edge> lt, Vertex start, Vertex end){
		...
		System.out.println(leadsTo.size());
		Vertex curr = end; 
		while (!leadsTo.get(curr).getNeighbor(curr).equals(start)) { // why is this part giving me an error
			path.add(curr);
			System.out.println("hii");
			curr = leadsTo.get(curr).getNeighbor(curr);
		}
		path.add(leadsTo.get(curr).getNeighbor(curr));
		return path;
